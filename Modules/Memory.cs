...
namespace ExSharpBase.Modules
{
    class Memory
    {
        ...
        public void LoadDriver()
        {
            if (!HasValidHandle())
            {
                string DriverPath = Application.StartupPath + "\\IODriver.sys";
                string kdmapperPath = Application.StartupPath + "\\kdmapper.exe";
                File.WriteAllBytes(DriverPath, ExSharpBase.Properties.Resources.kernelmode);
                File.WriteAllBytes(kdmapperPath, ExSharpBase.Properties.Resources.kdmapper);
                MappingDriver();
        ...
        private void MappingDriver(string driverName = "IODriver.sys")
        {
